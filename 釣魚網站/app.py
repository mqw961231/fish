...
# ⚠️ 注意：我移除了 POST 方法的部分，因為你的表單是用 JavaScript 前端處理的
# 不需要送到後端，所以 home() 只需要處理 GET 請求

if __name__ == "__main__":
    # 重要修改：讓 Flask 使用 Render 提供的 port
    port = int(os.environ.get("PORT", 5000))  # 如果沒有 PORT 環境變數，就用 5000
    app.run(host="0.0.0.0", port=port, debug=False)  # debug 要設為 False